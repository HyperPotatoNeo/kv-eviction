#!/usr/bin/env python3
"""Report success rates grouped by the number of Gaussian KV-noise events."""
...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "usage: scripts/noise_success_by_count.py <run-dir|results.jsonl>",
            file=sys.stderr,
        )
        return 2
    ...
    with results_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            row = json.loads(line)
            rows += 1
            buckets[_num_noise_events(row)].append(_is_success(row))

    print(f"Gaussian-KV-noise-count success rates for {results_path}")
    print("N_noise_events  successes  total  accuracy")
    for n in sorted(k for k in buckets if k is not None):
        ...
        print(f"{n:14d}  {successes:9d}  {total:5d}  {accuracy:8.3f}")
    if None in buckets:
        total = len(buckets[None])
        successes = sum(1 for ok in buckets[None] if ok)
        accuracy = successes / total if total else 0.0
        print(f"{'unknown':>14}  {successes:9d}  {total:5d}  {accuracy:8.3f}")
