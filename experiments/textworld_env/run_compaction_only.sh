#!/bin/bash
# run_compaction_only.sh — Slim compaction-only launcher for parallel runs.
#
# Mirror of run_full_context_only.sh but targets the compaction config and
# enables client-side block-16 padding.
#
# Submit:
...
CONTAINER="skyrl-textworld-cmponly"
...
echo ""
echo "=============================================="
echo "RUNNING: compaction (turn-based eviction)"
echo "=============================================="
ssh -o StrictHostKeyChecking=no "$COMPUTE_NODE" \
    "export HOME=$SCRATCH && export PODMANHPC_PODMAN_BIN=$PODMANHPC_PODMAN_BIN && \
