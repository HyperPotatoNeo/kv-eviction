...
# Share the runner script from the main launcher — same contract. Rewritten on
# every run so stale copies do not miss new eval args.
...
ssh -o StrictHostKeyChecking=no "$COMPUTE_NODE" \
    "export HOME=$SCRATCH && export PODMANHPC_PODMAN_BIN=$PODMANHPC_PODMAN_BIN && \
     ...
     $NUM_EXAMPLES $MAX_EPISODE_STEPS $MAX_CONCURRENT 0 $EVAL_SET_JSON"
