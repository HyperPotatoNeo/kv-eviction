...
echo "=== Smoke 1b: $(hostname) ==="
nvidia-smi -L | head -4

podman-hpc run --rm \
  --user "$(id -u):$(id -g)" --replace --name kv-smoke1b \
  ...
  bash -c '
    ...
    echo "--- Run 1: bptt_segments=1 (M3) ---"
    python smoke1b_per_segment_backward.py --num-events -1 --sample-idx 0 \
      --bptt-segments 1 --output results/smoke1b_k1.json
    echo ""
    echo "--- Run 2: bptt_segments=4 ---"
    python smoke1b_per_segment_backward.py --num-events -1 --sample-idx 0 \
      --bptt-segments 4 --output results/smoke1b_k4.json
    echo ""
    echo "--- Run 3: bptt_segments=12 ---"
    python smoke1b_per_segment_backward.py --num-events -1 --sample-idx 0 \
      --bptt-segments 12 --output results/smoke1b_k12.json
  '
echo "=== Smoke 1b done ==="
