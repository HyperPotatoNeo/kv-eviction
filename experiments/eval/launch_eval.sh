#!/bin/bash
# Launch compaction eval on Perlmutter interactive node.
#
# Usage:
#   1. Get a node:
#      salloc -A m5017 -C "gpu&hbm80g" --qos=interactive --time 4:00:00 --gpus-per-node 4 -N 1
#
#   2. Run this script ON the allocated node:
...
echo "Node: $(hostname)"
echo "GPUs: $(nvidia-smi -L 2>/dev/null | wc -l || echo 'N/A (login node?)')"
...
echo ""
echo "Done. Results at: /pscratch/sd/s/siddart2/kv-eviction/experiments/eval/results/"
