...
RUN_NAME="full_context_rgmix_${STAMP}"
...
{"experiment":"full_context_rgmix","run_name":"$RUN_NAME","run_dir":"$RUN_DIR","job_id":"$SLURM_JOB_ID","git_sha":"$GIT_SHA","status":"submitted","launch_mode":"interactive_allocation"}
EOF

bash "$SCRIPT_DIR/preflight.sh" full_context_rgmix --require-allocation

echo "Launching full-context baseline in allocation $SLURM_JOB_ID"
echo "  Run dir: $RUN_DIR"

export MILA_LAUNCH_MODE=interactive_allocation

bash "$SCRIPT_DIR/run_in_allocation.sh" full_context_rgmix "$RUN_DIR"
