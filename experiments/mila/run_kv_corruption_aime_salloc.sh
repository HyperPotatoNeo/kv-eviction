...
export KV_CORRUPTION_LAUNCH_STAMP="$SESSION_STAMP"
NUM_GPUS="${NUM_GPUS:-1}"
if [ "$NUM_GPUS" = "4" ]; then
  DEFAULT_INF_TOML="$REPO_DIR/experiments/kv_corruption_aime/inference_dp4.toml"
else
...
salloc \
  ...
  bash -lc '
    ...
    bash experiments/mila/run_aime_eval.sh kv_corruption real
  '
