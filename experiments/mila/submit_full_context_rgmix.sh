...
bash "$SCRIPT_DIR/preflight.sh" full_context_rgmix
...
SBATCH_CMD=(sbatch "${SBATCH_ARGS[@]}" "$SCRIPT_DIR/full_context_rgmix.sbatch" "$RUN_DIR")
...
{"experiment":"full_context_rgmix","run_name":"$RUN_NAME","run_dir":"$RUN_DIR","job_id":"$JOB_ID","git_sha":"$GIT_SHA","status":"submitted"}
EOF

echo "Submitted full-context baseline."
echo "  Job ID:  $JOB_ID"
