...
NUM_GPUS="${NUM_GPUS:-1}"
if [ "$NUM_GPUS" = "4" ]; then
  DEFAULT_INF_TOML="$REPO_DIR/experiments/kv_corruption_aime/inference_full_context_dp4.toml"
else
  DEFAULT_INF_TOML="$REPO_DIR/experiments/kv_corruption_aime/inference_full_context.toml"
fi
...
salloc \
  ...
  bash -lc '
    ...
    bash experiments/mila/run_aime_eval.sh full_context real
  '
