#!/bin/bash
# full_context_rgmix — full-context baseline (no compaction), matching
# compaction_rgmix config otherwise. 3-node 2-1 split:
#   Node 0: vLLM DP=4 inference (no compaction)
#   Node 1: vLLM DP=4 inference (no compaction)
#   Node 2: FSDP2 DP=4 trainer
#
# Submit via:
#   sbatch -A m4881 -C "gpu&hbm80g" --qos=premium --time 48:00:00 \
#          --gpus-per-node 4 --nodes=3 \
#          experiments/full_context_rgmix/launch.sh
set -e
set -o pipefail  # so `ssh ... | tee` propagates the ssh/trainer exit
                 # status into $PIPESTATUS[0] below, not tee's always-0.

#SBATCH --job-name=full_context_rgmix
#SBATCH --output=/pscratch/sd/s/siddart2/kv-eviction/experiments/full_context_rgmix/slurm_%j.out
#SBATCH --error=/pscratch/sd/s/siddart2/kv-eviction/experiments/full_context_rgmix/slurm_%j.err

module unload darshan 2>/dev/null || true

SCRATCH=/pscratch/sd/s/siddart2
KV_DIR="$SCRATCH/kv-eviction"
EXP_DIR="$KV_DIR/experiments/full_context_rgmix"
CONTAINER_IMAGE="docker.io/novaskyai/skyrl-train-ray-2.51.1-py3.12-cu12.8"
CONTAINER_INF0="skyrl-fcrg-inf0"
CONTAINER_INF1="skyrl-fcrg-inf1"
CONTAINER_TRAIN="skyrl-fcrg-train"
...
echo "==========================================="
echo "full_context_rgmix baseline (3 nodes, 48h)"
echo "==========================================="
...
echo "==========================================="
echo "full_context_rgmix launched!"
