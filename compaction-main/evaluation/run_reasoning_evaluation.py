...
class ReasoningEvaluator:
    ...
    def _kv_to_compacted_format(
        ...
        """Convert standard KV cache to compacted (C1, beta, C2) format with zero beta."""
        if isinstance(kv_cache, DynamicCache):
            kv_layers = tuple((layer.keys, layer.values) for layer in kv_cache.layers)
        else:
            kv_layers = kv_cache

        result = []
        for layer_idx in range(len(kv_layers)):
            k = kv_layers[layer_idx][0]
            v = kv_layers[layer_idx][1]
...
def main():
    ...
    max_model_len = 131072 if args.long_context else args.max_seq_len
